...
- `jaccard-top5-graph_2024-03-27.csv.gz`:
  Derived from the above files.
  The three columns are source, target, and Jaccard similarity of the go sets.
  In this graph I only keep the top 5 targets for each source (like they did in the paper).
  Some sources get more than 5 targets when more than 5 genes are at least as similar as the top 5th.
  I used HTCondor (submit file `compute_jaccard_subsets.submit`) and then merged the results.
