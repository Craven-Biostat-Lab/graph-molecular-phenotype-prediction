...
rf_auc_df = pd.DataFrame(index =[train_list[i][0]for i in range(len(train_list))],columns= [test_list[i][0]for i in range(len(test_list))])
rf_prc_df = pd.DataFrame(index =[train_list[i][0]for i in range(len(train_list))],columns= [test_list[i][0]for i in range(len(test_list))])
rf_f1_df = pd.DataFrame(index =[train_list[i][0]for i in range(len(train_list))],columns= [test_list[i][0]for i in range(len(test_list))])
        ...
        if test_name == train_name:
            ...
            if test_name != 'Mitomics':
                ...
                for i, (train_index, test_index) in enumerate(skf.split(data.iloc[:,:-1], data.iloc[:,-1])):
                    rf_config = json.load(open(DATA_DIREC+'/config/'+train_name+'/best_paras_'+train_name+'_RandomForest_subset_False_fold_'+str(i)+'_ratio_1.0_5fold.json'))
            ...
            rf_config = json.load(open(DATA_DIREC+'/config/'+train_name+'/best_paras_'+train_name+'_transfer_st_relation_RandomForest_transfer.json'))
            seed_everything()
            model = RandomForestClassifier(**rf_config,class_weight = 'balanced')
            model.fit(X_train, y_train)
...
rf_auc_df.to_csv(PLOT_DIREC+'/Transfer/Transfer_AUROC_RandomForest_st_relation.csv')
rf_prc_df.to_csv(PLOT_DIREC+'/Transfer/Transfer_AUPRC_RandomForest_st_relation.csv')
...
for train_dataset in train_list:
    ...
    for test_dataset in test_list:
        ...
        if test_name == train_name:
            ...
            X_test = test_pheno_data.iloc[:, :-1]
            y_test = test_pheno_data.iloc[:, -1]
            rf_config = json.load(open(DATA_DIREC+'/config/'+train_name+'/best_paras_'+train_name+'_transfer_RandomForest_transfer.json'))
            seed_everything()
            model = RandomForestClassifier(**rf_config,class_weight = 'balanced')
            model.fit(X_train, y_train)
            y_prob = model.predict_proba(X_test)[:, 1]
...
rf_auc_df.to_csv(PLOT_DIREC+'/Transfer/Transfer_AUROC_RandomForest_all_features.csv')
rf_prc_df.to_csv(PLOT_DIREC+'/Transfer/Transfer_AUPRC_RandomForest_all_features.csv')
...
for train_dataset in train_list:
    ...
    for test_dataset in test_list:
        ...
        if test_name == train_name:
            ...
            X_test = test_pheno_data.iloc[:, :-1]
            y_test = test_pheno_data.iloc[:, -1]
            rf_config = json.load(open(DATA_DIREC+'/config/'+train_name+'/best_paras_'+train_name+'_transfer_ppi_only_RandomForest_transfer.json'))
            seed_everything()
            model = RandomForestClassifier(**rf_config,class_weight = 'balanced')
            model.fit(X_train, y_train)
            y_prob = model.predict_proba(X_test)[:, 1]
...
rf_auc_df.to_csv(PLOT_DIREC+'/Transfer/Transfer_AUROC_RandomForest_ppi_only.csv')
rf_prc_df.to_csv(PLOT_DIREC+'/Transfer/Transfer_AUPRC_RandomForest_ppi_only.csv')
